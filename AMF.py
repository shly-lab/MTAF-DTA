...
def morgan_binary_features_generator(mol: Molecule,
    ...
    return features
def Avalon_features_generator(mol: Molecule,
                                     radius: int = MORGAN_RADIUS,
                                     num_bits: int = MORGAN_NUM_BITS) -> np.ndarray:
    """
    Generates a binary Morgan fingerprint for a molecule.
...
class GNNDataset(InMemoryDataset):
    ...
    def process_data(self, data_path):
        ...
        for i, row in df.iterrows():
            ...
            if mol == None:
                ...
                continue
            morgan_feature = morgan_binary_features_generator(mol)
            Avalon_feature=Avalon_features_generator(mol)
            ...
            data = DATA.Data(
                ...
                smi=smi,
                morgan=morgan_feature,
                Avalon=Avalon_feature,
    ...
    def process_train_data(self, data_path):
        ...
        for i, row in df.iterrows():
            ...
            # print("morgan shape:",morgan_feature.shape)  #(2084, )
            x_list, edge_index_list, edge_attr_list = mol_to_graph(mol, times=4)
